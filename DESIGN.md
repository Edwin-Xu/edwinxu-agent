...
### MCP Server 鉴权（第三方/服务对服务，无登录态）
本项目推荐把 MCP server 放在 `agent-api` 之后：Web/CLI/第三方 **只调用 `agent-api`**，由 `agent-api` 作为 **MCP client + 工具网关** 统一做鉴权、限流与审计；避免客户端直连 MCP server 造成权限分散与审计断裂。

按“通用性/可扩展性/对接成本”综合排序，主流方案如下（从推荐到备选）：

#### 方案 A：OAuth2 Client Credentials（推荐）
适用于：多第三方、多权限（scope）、需要可撤销与可审计的生产环境。
- **机制**：第三方用 `client_id/client_secret` 换取短期 `access_token`，再用 `Authorization: Bearer <token>` 调用接口
- **优点**：可设置过期、撤销、scope/权限隔离、按 client 统计与审计；后续扩展第三方不混乱
- **代价**：需要一个授权服务（可用现成 IdP：Auth0/Keycloak/自建 OIDC）

#### 方案 B：每个第三方一把 API Key / 固定 Bearer Token（最小实现）
适用于：第三方数量少、希望快速上线、可接受“静态密钥”的风险。
- **机制**：第三方请求携带 `Authorization: Bearer <固定key>`（或 `X-API-Key`）
- **必须配套**：
  - **按第三方分配不同 key**（不要全员共用一把）
  - **可轮换/可作废**（泄露可快速切断）
  - **限流与配额**（按 key 维度）
  - **审计日志**（至少记录：key 标识、接口、tool、耗时、结果摘要；敏感字段脱敏）
  - 可选：**IP allowlist**（若第三方出口 IP 稳定）

#### 方案 C：HMAC 请求签名（常用于 webhook/回调）
适用于：需要抗重放、或不希望长期 Bearer key 明文常驻请求头的场景（例如 webhook）。
- **机制**：共享 secret；请求带 `timestamp + nonce + body` 的签名（例如 `X-Signature`）
- **优点**：配合时间窗可有效防重放；泄露面更小
- **代价**：对接成本更高（双方都要实现签名与重放窗口校验）

#### 额外选项：mTLS（有条件再上）
适用于：企业内网、强合规、基础设施团队可统一发证与轮换的场景。
- **机制**：双向 TLS 证书校验服务身份
- **优点**：强身份、无需应用层 token
- **代价**：证书生命周期管理成本高

**落地建议（MVP → 生产）**
- MVP/内测：优先 **方案 B**（固定 Bearer/API Key）+ 限流 + 审计
- 面向多第三方/生产：升级到 **方案 A**（Client Credentials）并用 scope 限制哪些 client 可以“管理 MCP 配置”、哪些只能“调用只读 tools”
- webhook：优先 **方案 C**（HMAC）并配合 IP allowlist（可用就用）
